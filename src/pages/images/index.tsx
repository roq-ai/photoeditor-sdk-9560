...
import { getImages, deleteImageById } from 'apiSdk/images';
...
function ImageListPage() {
  ...
  const handleView = (id: string) => {
    if (hasAccess('image', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
  ...
  return (
    <AppLayout>
      <Box bg="white" p={4} rounded="md" shadow="md">
        {hasAccess('image', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <Flex justifyContent="space-between" mb={4}>
            <Text as="h1" fontSize="2xl" fontWeight="bold">
              Image
            </Text>
            <NextLink href={`/images/create`} passHref legacyBehavior>
                  ...
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    <Td>
                      {hasAccess('image', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                        <NextLink href={`/images/edit/${record.id}`} passHref legacyBehavior>
                          <Button
